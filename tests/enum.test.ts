...
describe("EnumShape", () => {
  test("basic enum validation (string)", () => {
    const schema = t.enum(["red", "green", "blue"] as const);
  ...
  test("basic enum validation (number)", () => {
    const schema = t.enum([1, 2, 3] as const);
  ...
  test("optional enum", () => {
    const schema = t.enum(["a", "b"] as const).optional();
  ...
  test("nullable enum", () => {
    const schema = t.enum(["a", "b"] as const).nullable();
    expect(schema.parse("a")).toBe("a");
  ...
  test("hasValue", () => {
    const schema = t.enum(["a", "b", "c"] as const).hasValue("b");
    expect(schema.parse("b")).toBe("b");
  ...
  test("notValue", () => {
    const schema = t.enum(["a", "b", "c"] as const).notValue("b");
    expect(schema.parse("a")).toBe("a");
  ...
  test("oneOf", () => {
    const schema = t.enum(["a", "b", "c", "d"] as const).oneOf(["a", "b"]);
    expect(schema.parse("a")).toBe("a");
    expect(schema.parse("b")).toBe("b");
    expect(() => schema.parse("c")).toThrow();
  ...
  test("notOneOf", () => {
    const schema = t.enum(["a", "b", "c", "d"] as const).notOneOf(["a", "b"]);
    expect(schema.parse("c")).toBe("c");
  ...
  test("chained validations", () => {
    const schema = t.enum(["a", "b", "c", "d"] as const)
      .notValue("a")
      .oneOf(["b", "c"]);
  ...
  test("error messages", () => {
    const schema = t.enum(["a", "b"] as const);
    try {
      schema.parse("c");
    } catch (error) {
  ...
  test("complex enum with mixed types", () => {
    const schema = t.enum(["text", 42, true] as const);
